...
#include "ui_mb_endpage.h"
#include "clockbus.h"
#include <QStackedWidget>
#include "global.h"

mb_EndPage::mb_EndPage(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::mb_EndPage)
{
    ui->setupUi(this);
...
mb_EndPage::~mb_EndPage()
{
    delete ui;
}
    void mb_EndPage::showEvent(QShowEvent *event)
{
    ...
    // ui->carbon_lb->setText(QString::number(global::carbonReduction(), 'f', 1));
    // ui->tree_lb->setText(QString::number(global::treeCount()));
    ui->mbName_lb->setText(global::username);
    startCountdown(60);
}

void mb_EndPage::hideEvent(QHideEvent *event)
{
    // 全域變數歸零
    global::carbonReduction() = 0.0;
    global::treeCount() = 0;

    ui->carbon_lb->setText("000.0");
    ui->tree_lb->setText("0000");
    countDownTimer->stop();
}

void mb_EndPage::mousePressEvent(QMouseEvent *event)
{
    startCountdown(60);
}

void mb_EndPage::startCountdown(int seconds)
{
    countdownValue = seconds;
    ui->countdown_lb->setText(QString::number(countdownValue));
    // 確保計時器不會重複連線
    disconnect(countDownTimer, nullptr, this, nullptr);
    connect(countDownTimer, &QTimer::timeout, this, &mb_EndPage::updateCountLabel);
    countDownTimer->start(1000);  // 每 1 秒觸發一次
}

void mb_EndPage::updateCountLabel()
{
    countdownValue -= 1;
    if(countdownValue <= -1 )
    {
...
void mb_EndPage::on_backhome_btn_clicked()
{
